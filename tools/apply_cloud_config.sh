...
MIRROR_BUCKET="${MIRROR_BUCKET:-profbailey-mirror}"
...
gcloud storage buckets update "gs://${MIRROR_BUCKET}" \
  --project="${PROJECT_ID}" \
  --default-storage-class=ARCHIVE \
  --lifecycle-file="${ROOT_DIR}/infra/mirror.lifecycle.json"

gcloud storage objects update "gs://${MIRROR_BUCKET}/mirror.zip" \
...
if [[ "${PRIVATE_STORAGE:-0}" == "1" ]]; then
  gcloud storage buckets remove-iam-policy-binding "gs://${ARCHIVE_BUCKET}" \
    ...
    --quiet || true
  gcloud storage buckets remove-iam-policy-binding "gs://${MIRROR_BUCKET}" \
    --member=allUsers \
    --role=roles/storage.objectViewer \
    --quiet || true
  gcloud storage buckets update "gs://${ARCHIVE_BUCKET}" --public-access-prevention --project="${PROJECT_ID}"
  gcloud storage buckets update "gs://${MIRROR_BUCKET}" --public-access-prevention --project="${PROJECT_ID}"
else
